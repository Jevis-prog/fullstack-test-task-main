...
- Подключение к БД вынесено из сервисного слоя в `src/database.py`;
- загрузка файла выполняется частями, при ошибке незавершенный файл удаляется;
- последовательность из трех Celery-задач объединена в одну: обработка использует одно чтение из БД и один commit;
- исправлено удаление файлов, для которых уже созданы алерты;
- добавлена базовая валидация входных данных и тесты правил проверки файлов;
- фронтенд разделен на API-слой, типы данных, форматирование, управление состоянием и представление.
